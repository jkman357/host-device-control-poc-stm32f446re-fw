...
def run_hardware_sweep(
        ...
        try:
            with KeyPoller() as key_poller:
                while True:
                    ...
                    # Check the keyboard after one receive window so an immediate q/Q
                    # still allows the first telemetry sample to prove the stream path.
    ...
    if statistics.telemetry_frames == 0:
        raise RuntimeError(
            "no TELEMETRY_SAMPLE frame was received; verify that the MCU startup "
            "enabled CP10/CP11 before hard-float telemetry code executes"
        )
